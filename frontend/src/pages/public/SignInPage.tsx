      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h2 className="text-xl font-bold font-display text-[var(--color-text)]">
            Welcome back to ScenoraEdits
          </h2>
          <p className="text-xs text-[var(--color-text-secondary)] mt-1">
            Access your AI video projects, bibles, and production studio.
          </p>
        </div>
        ...
        <Card variant="default" className="p-6 sm:p-8 shadow-[var(--shadow-card)]">
          {!isConfigured && (
            <Alert type="warning" className="mb-5" title="Firebase Setup Needed">
              Real authentication requires Firebase keys. Add <code className="font-mono text-[10px]">VITE_FIREBASE_API_KEY</code>, etc. to <code className="font-mono text-[10px]">frontend/.env</code>.
            </Alert>
          )}

          {displayError && (
            <Alert type="error" className="mb-5" title="Sign In Error" onDismiss={() => { setLocalError(null); clearError(); }}>
              {displayError}
            </Alert>
          )}
